...
def add_admin():
    """Add a Google user as admin. Use old admin token to authorize."""
    auth = request.headers.get('Authorization', '')
    if not auth.startswith('Bearer '):
        return jsonify({'error': 'Unauthorized'}), 401
    admin_token = auth[7:]
    # Verify old admin token
    with get_db() as db:
        cur = db.cursor()
        cur.execute('SELECT token FROM users WHERE id = ?', ('admin',))
        row = cur.fetchone()
    ...
    with get_db() as db:
        db.execute('INSERT OR REPLACE INTO admins (google_id, email, name, added_at) VALUES (?, ?, ?, ?)',
...
def list_admins():
    ...
    with get_db() as db:
        cur = db.cursor()
        cur.execute('SELECT google_id, email, name, added_at FROM admins ORDER BY added_at DESC')
        rows = cur.fetchall()
    return jsonify({'admins': [{'google_id': r['google_id'], 'email': r['email'], 'name': r['name'], 'added_at': r['added_at']} for r in rows]})
