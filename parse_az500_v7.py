#!/usr/bin/env python3
"""
AZ-500 507题 — Final Parser (v7)
Handles options A-Z (not just A-E), multi-select answers, note/does formats.
"""
...
def parse_block(topic, qnum, block):
    ...
    # Find Correct Answer: multi-letter allowed (e.g. "AB", "BF")
    # Can be: "Correct Answer:BF\n" or "Correct Answer: BF\n" or "Correct Answer:BF\nB"
    # Be careful: "Correct Answer:AC" where next char is letter = "Correct Answer:AC" + "Reference"
    # The answer is the FIRST group of letters after "Correct Answer:"
    ca_match = re.search(r'Correct Answer:\s*([A-Z]{1,6})\s*(?:\n|$)', block)
    if not ca_match:
        return None, 'NO_CORRECT_ANSWER'

    answer_str = ca_match.group(1)  # e.g. "A" or "BF"
    ca_pos = ca_match.start()
    before_ca = block[:ca_pos]
    after_ca = block[ca_match.end():].strip()

    # Extract question text and options
    # Options pattern: "[LETTER]. [text]" where letter can be A-Z
    # Handle "F . Upload..." pattern (space before dot = "F .")
    # Handle "F . Upload..." (space before dot = OCR artifact)
    # Also handle "F. Upload..." (normal)
    opt_pattern = re.compile(r'^([A-Z])\s*\.\s+(.*)$', re.MULTILINE)
...
opt_dist = C(len(q['options']) for q in questions)
print("\nOptions count distribution:")
for n in sorted(opt_dist.keys()):
    print(f"  {n} options: {opt_dist[n]}")